 ...
 *
 * Ported from web `src/session/composeSession.test.ts`; vitest's
 * `expect(value, message)` second-argument form is unsupported by jest's typed
 * `expect`, so the inline failure messages were dropped — the assertions
