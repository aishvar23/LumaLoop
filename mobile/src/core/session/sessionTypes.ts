// Ported from web `src/session/sessionTypes.ts`; source of truth is the web app
// — keep in sync (Phase M, M2). Ported because `composeSession`/`feedDeck` depend
// on `SessionMode` + `MODE_DEFAULTS`; it is pure and DOM-free. The session
// reducer/controller themselves are out of scope for M2. See `mobile/src/core/README.md`.
/**
