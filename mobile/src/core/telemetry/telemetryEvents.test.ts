...
describe('TelemetryEventNames (FEED_DIRECTION §6 contract)', () => {
  ...
  it('is frozen so the canonical event set cannot be mutated at runtime', () => {
    ...
    // The freeze must hold whether the module body runs strict (the web ES
    // module — where this assignment throws) or sloppy (the RN/Expo babel
    // transform emits non-strict CommonJS — where the write silently no-ops).
    // We assert the EFFECT (no new key, length unchanged) rather than the
    // strict-mode throw the web suite relied on, so the contract is pinned in
    // either mode. A silent no-op that ADDED the key would still fail here.
    try {
      (TelemetryEventNames as Record<string, string>).New_Event = 'New_Event';
    } catch {
      // A strict-mode TypeError is acceptable; the assertions below are the test.
    }
    expect(
      (TelemetryEventNames as Record<string, string>).New_Event,
    ).toBeUndefined();
    expect(Object.keys(TelemetryEventNames)).toHaveLength(9);
  });
});
