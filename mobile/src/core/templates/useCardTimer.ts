 ...
 *
 * Every template renderer mounts this hook to arm a per-card countdown for its
 * card's `timeLimitMs`. On expiry the hook resolves the card as a TIMEOUT via the
 * shared {@link buildTimeoutResolution} helper, so timeout behavior is implemented
 * once here instead of re-derived per renderer.
 *
