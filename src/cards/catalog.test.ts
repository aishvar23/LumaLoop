...
describe('authored card catalog', () => {
  ...
  it('has a strong hard-tier ceiling for the difficulty ramp', () => {
    ...
    // the ramp from repeating. Spot It is intentionally entry-level now, and
    // code_break is intentionally extremely-hard-only, so both are excluded from
    // the per-template hard-card requirement.
    const templates: TemplateType[] = [
      'what_changed',
      'rule_flip',
      'tiny_logic',
  ...
  it('mixes difficulty within each non-entry template', () => {
    ...
    for (const template of templates) {
      ...
      expect(
        perTemplate.size,
        `template "${template}" should mix difficulties, got ${[...perTemplate].join(', ')}`,
  ...
  it('maps every card category to its template in templateCategoryMap', () => {
    for (const card of catalog) {
      expect(
        templateCategoryMap[card.templateType],
        `card ${card.cardId} category "${card.category}" must be valid for ${card.templateType}`,
  ...
  it('keeps every sequence prompt neutral until the card is solved', () => {
    ...
    for (const card of sequenceCards) {
      expect(
        neutralPrompts,
        `card ${card.cardId} must ask for the answer without revealing its pattern`,
      ).toContain(card.prompt);
      expect(card.explanation.body.trim().length).toBeGreaterThan(0);
    }
  });
  ...
  it('keeps spot_it anomalies distinct in glyph, not color alone', () => {
    ...
    for (const card of spotIt) {
      ...
      expect(
        ...
        `spot_it card ${card.cardId} base and anomaly must differ in glyph`,
      ).not.toBe(card.config.anomalyElement);
    }
  });
  ...
  it('keeps spot_it anomalies visually distinguishable in the app font', () => {
    ...
    for (const card of spotIt) {
      ...
      expect(
        ...
        `spot_it card ${card.cardId} uses a visually confusable odd-one-out pair`,
      ).toBe(false);
    }
  });
