/**
 * Tests for the pure Code Break evaluator (Azure DevOps #143).
 *
 * Pure-function tests: no React, no timers. The peg-counting suite focuses on
 * the duplicate-symbol edge cases that are the classic Mastermind bug source (a
 * symbol must never be counted in both exact and partial, nor counted twice),
 * plus win/lose resolution and the derived signals.
 */
