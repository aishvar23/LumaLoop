/**
 * Tests for the Pattern Chain renderer (Azure DevOps #138; Tech §6, §7, §14).
 *
...
const pick = (optionId: string) =>
  act(() => void fireEvent.click(screen.getByTestId(`pc-option-${optionId}`)));
...
describe('step flow', () => {
  it('shows the visible sequence and the first step options on mount', () => {
    ...
    expect(screen.getByTestId('pc-option-s1-a')).toBeInTheDocument();
    expect(screen.queryByTestId('pc-option-s2-a')).not.toBeInTheDocument();
  });
  ...
  it('advances to the next step on a pick, appending the chosen item', () => {
    ...
    expect(screen.getByTestId('pc-option-s2-a')).toBeInTheDocument();
    expect(screen.queryByTestId('pc-option-s1-a')).not.toBeInTheDocument();
  ...
  it('announces step progress in a polite live region', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Picked: 8');
  });
});
